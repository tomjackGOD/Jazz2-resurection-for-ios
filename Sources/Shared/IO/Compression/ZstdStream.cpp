...
//###==##====#=====--==~--~=~- --- -- -  -  -   -
	...
	void ZstdStream::InitializeInternal()
	{
		...
		if (ZSTD_isError(result)) {
			...
			LOGE("ZSTD_initDStream() failed with error 0x{:x} ({})", result, ZSTD_getErrorName(result));
#endif
			_state = State::Failed;
		}
	...
	std::int32_t ZstdStream::ReadInternal(void* ptr, std::int32_t size)
	{
		...
		while (n == 0) {
			...
			if (ZSTD_isError(result)) {
#if defined(DEATH_TRACE_VERBOSE_IO)
				LOGE("ZSTD_decompressStream() failed with error 0x{:x} ({})", result, ZSTD_getErrorName(result));
#endif
				_state = State::Failed;
				return Stream::Invalid;
			}
		...
		: _outputStream(&outputStream), _state(State::Created), _outBufferPos(0), _outBufferLength(0)
	{
		...
		if (ZSTD_isError(result)) {
#if defined(DEATH_TRACE_VERBOSE_IO)
			LOGE("ZSTD_initCStream() failed with error 0x{:x} ({})", result, ZSTD_getErrorName(result));
#endif
			_state = State::Failed;
		}
	...
	std::int32_t ZstdWriter::WriteInternal(const void* buffer, std::int32_t bytesToWrite, bool finish)
	{
		if (bytesToWrite > 0) {
			...
			if (ZSTD_isError(result)) {
#if defined(DEATH_TRACE_VERBOSE_IO)
				LOGE("ZSTD_compressStream() failed with error 0x{:x} ({})", result, ZSTD_getErrorName(result));
#endif
				_state = State::Failed;
				return Stream::Invalid;
			}
		...
		if (finish) {
			...
			if (ZSTD_isError(result)) {
#if defined(DEATH_TRACE_VERBOSE_IO)
				LOGE("ZSTD_endStream() failed with error 0x{:x} ({})", result, ZSTD_getErrorName(result));
#endif
				_state = State::Failed;
				return Stream::Invalid;
			}
