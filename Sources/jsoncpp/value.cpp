...
// static
	...
	static inline char* duplicateStringValue(const char* value, size_t length) {
		...
		if (newString == nullptr) {
			throwRuntimeError("in Json::Value::duplicateStringValue(): Failed to allocate string value buffer");
		}
...
namespace Json {
	...
	Exception::Exception(StringContainer msg) : msg_(std::move(msg)) {
	}
	Exception::~Exception() noexcept = default;
	char const* Exception::what() const noexcept {
		return msg_.c_str();
	}
	RuntimeError::RuntimeError(StringContainer const& msg) : Exception(msg) {
	}
	LogicError::LogicError(StringContainer const& msg) : Exception(msg) {
	}
	...
	Value::Value(const StringContainer& value) {
		initBasic(stringValue, true);
		value_.string_ = duplicateAndPrefixStringValue(
...
#ifdef JSONCPP_HAS_STRING_VIEW
	ErrorCode Value::get(std::string_view& view) const {
		if (type() != stringValue)
		...
		unsigned length;
		decodePrefixedString(this->isAllocated(), this->value_.string_, &length,  &begin);
		view = std::string_view(begin, length);
		return SUCCESS;
	}
#endif

	StringContainer Value::asString() const {
		switch (type()) {
			case nullValue:
				return "";
			case stringValue: {
				...
				char const* this_str;
				decodePrefixedString(this->isAllocated(), this->value_.string_, &this_len, &this_str);
				return StringContainer(this_str, this_len);
			}
			case booleanValue:
	...
	Value::UInt Value::asUInt() const {
		switch (type()) {
			...
			case realValue:
				JSON_ASSERT_MESSAGE(InRange(value_.real_, 0u, maxUInt), "double out of UInt range");
	...
	ErrorCode Value::get(Value::Int64& value) const {
		switch (type()) {
			case intValue:
				value = Int64(value_.int_);
				return SUCCESS;
			case uintValue:
				if (!isInt64()) {
					return BIGINT_ERROR;
				}
				value = Int64(value_.uint_);
				return SUCCESS;
			case realValue:
				...
				if (value_.real_ == minInt64 || !InRange(value_.real_, minInt64, maxInt64)) {
					return BIGINT_ERROR;
				}
	...
	Value::UInt64 Value::asUInt64() const {
		switch (type()) {
			...
			case realValue:
				JSON_ASSERT_MESSAGE(InRange(value_.real_, 0u, maxUInt64), "double out of UInt64 range");
	...
	ErrorCode Value::get(bool& value) const {
		switch (type()) {
			case booleanValue:
				value = value_.bool_;
				return SUCCESS;
			...
			case intValue:
				...
				return SUCCESS;
			case uintValue:
				value = value_.uint_ != 0;
				return SUCCESS;
			case realValue: {
				// According to JavaScript language zero or NaN is regarded as false
				const auto value_classification = std::fpclassify(value_.real_);
				value = value_classification != FP_ZERO && value_classification != FP_NAN;
				return SUCCESS;
			}
			default:
				return INCORRECT_TYPE;
	...
	Value const* Value::find(const StringContainer& key) const {
		return find(key.data(), key.data() + key.length());
	}

	Value const* Value::findNull(const StringContainer& key) const {
		return findValue<Value, &Value::isNull>(key);
	}
	Value const* Value::findBool(const StringContainer& key) const {
		return findValue<Value, &Value::isBool>(key);
	}
	Value const* Value::findInt(const StringContainer& key) const {
		return findValue<Value, &Value::isInt>(key);
	}
	Value const* Value::findInt64(const StringContainer& key) const {
		return findValue<Value, &Value::isInt64>(key);
	}
	Value const* Value::findUInt(const StringContainer& key) const {
		return findValue<Value, &Value::isUInt>(key);
	}
	Value const* Value::findUInt64(const StringContainer& key) const {
		return findValue<Value, &Value::isUInt64>(key);
	}
	Value const* Value::findIntegral(const StringContainer& key) const {
		return findValue<Value, &Value::isIntegral>(key);
	}
	Value const* Value::findDouble(const StringContainer& key) const {
		return findValue<Value, &Value::isDouble>(key);
	}
	Value const* Value::findNumeric(const StringContainer& key) const {
		return findValue<Value, &Value::isNumeric>(key);
	}
	Value const* Value::findString(const StringContainer& key) const {
		return findValue<Value, &Value::isString>(key);
	}
	Value const* Value::findArray(const StringContainer& key) const {
		return findValue<Value, &Value::isArray>(key);
	}
	Value const* Value::findObject(const StringContainer& key) const {
	...
	Value const& Value::operator[](const StringContainer& key) const {
	...
	Value& Value::operator[](const StringContainer& key) {
		return resolveReference(key.data(), key.data() + key.length());
	}
#endif
	...
	Value Value::get(StringContainer const& key, Value const& defaultValue) const {
		return get(key.data(), key.data() + key.length(), defaultValue);
	}
#endif
	...
	bool Value::removeMember(StringContainer const& key, Value* removed) {
		return removeMember(key.data(), key.data() + key.length(), removed);
	}
#endif
	...
	void Value::removeMember(const StringContainer& key) {
	...
	bool Value::isMember(StringContainer const& key) const {
		return isMember(key.data(), key.data() + key.length());
	}
#endif
	...
	bool Value::Comments::hasAny() const {
		if (ptr_) {
			for (std::size_t slot = 0; slot < CommentPlacement::numberOfCommentPlacement; slot++) {
				if (!(*ptr_)[slot].empty())
					return true;
			}
		}
	...
	StringContainer Value::Comments::get(CommentPlacement slot) const {
		if (!ptr_)
			return {};
		return (*ptr_)[slot];
	}

	void Value::Comments::set(CommentPlacement slot, StringContainer comment) {
	...
	void Value::setComment(StringContainer comment, CommentPlacement placement) {
	...
	StringContainer Value::toStyledString() const {
		StreamWriterBuilder builder;

		StringContainer out = this->hasComment(commentBefore) ? "\n" : "";
	...
	PathArgument::PathArgument(StringContainer key) : key_(std::move(key)), kind_(kindKey) {
	}

	// class Path
	// //////////////////////////////////////////////////////////////////

	Path::Path(const StringContainer& path, const PathArgument& a1, const PathArgument& a2,
	...
	void Path::makePath(const StringContainer& path, const InArgs& in) {
		...
		while (current != end) {
			if (*current == '[') {
				...
				while (current != end && !strchr("[.", *current))
					++current;
				args_.push_back(StringContainer(beginName, current));
			}
		}
	}

	void Path::addPathInArg(const StringContainer& /*path*/, const InArgs& in,
	...
	void Path::invalidPath(const StringContainer& /*path*/, int /*location*/) {
		// Error: invalid path.
	}
