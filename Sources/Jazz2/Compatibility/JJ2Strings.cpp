...
namespace Jazz2::Compatibility
{
	bool JJ2Strings::Open(StringView path)
	{
		...
		std::uint32_t offsetArraySize = s->ReadValueAsLE<std::uint32_t>();

		std::uint32_t textArraySize = s->ReadValueAsLE<std::uint32_t>();
		...
		for (std::uint32_t i = 0; i < offsetArraySize; i++) {
			std::uint32_t offset = s->ReadValueAsLE<std::uint32_t>();
			CommonTexts.emplace_back(&textArray[offset]);
		}

		std::uint32_t levelEntryCount = s->ReadValueAsLE<std::uint32_t>();
		...
		for (std::uint32_t i = 0; i < levelEntryCount; i++) {
			...
			offsets.emplace_back(s->ReadValueAsLE<std::uint16_t>());
		}

		std::uint32_t textArray2Size = s->ReadValueAsLE<std::uint32_t>();
